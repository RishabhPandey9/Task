  ...
  return (
    ...
    <div className={!currentState?.show?'bg-white flex  space-x-1 text-xs md:text-xs lg:text-xl  font-semibold p-1 text-gray-600  md:p-2 ':"bg-white space-x-1 flex  lg:space-x-4 text-xs md:text-lg xl:text-xl  font-semibold p-1 text-gray-600  md:p-2 "}>
        ...
        <div className="md:flex mb-4 ml-4 md:ml-0 lg:mb-0 py-3 lg:pb-5  xl:pb-3">
          <div className="flex">
            <span className="text-base mt-3 lg:mt-4 xl:mt-3 text-slate-400 mr-2 md:ml-10">From&nbsp;:</span>
            <input
              onChange={(e) => {
                setFromDate(e.target.value);
              }}
              className="mt-1  md:mr-10 lg:mr-4 xl:mr-10   w-40 px-5 py-2 bg-white border shadow-sm border-slate-300 placeholder-text-bold placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block rounded-2xl sm:text-sm focus:ring-1"
              type="date"
              placeholder=""
            />
          </div>
          <div className="flex mt-2 md:mt-0">
            <span className="text-base mr-5 md:mr-0 mt-3 lg:mt-4 xl:mt-3 text-slate-400 ">To&nbsp;:</span>
            <input
               onChange={(e) => {
                setToDate(e.target.value);
            
              }}
              className="mt-1 ml-2  placeholder-text-sm w-40 px-5 py-2 bg-white border shadow-sm border-slate-300 placeholder-text-bold placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block  rounded-2xl sm:text-sm focus:ring-1"
              type="date"
              placeholder=""
            />
          </div>
        ...
        <div className={!currentState?.show?'mt-1 mr-2 md:mt-3 lg:mt-4 md:mr-3 lg:mr-10':'mt-1 mr-2 md:mt-4 md:mr-10'} >
            <Link to="/SendMessage">
            <Button className='w-auto px-5' size="small" variant="contained">Send message</Button>
          ...
          <div className="modal-dialog modal-dialog-centered relative w-auto pointer-events-none">
            <div className="modal-content border-none shadow-lg relative flex flex-col w-full pointer-events-auto bg-white bg-clip-padding rounded-md outline-none text-current">
              ...
              <div className="modal-header flex flex-shrink-0 items-center p-4  rounded-t-md">
                ...
                <div
                  className="text-xl ml-4 font-medium leading-normal text-gray-800"
